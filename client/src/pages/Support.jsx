...
import { MessageSquare, HelpCircle, Headphones, ChevronDown, Plus,
         Clock, CheckCircle, AlertCircle, Search, Package } from 'lucide-react'
...
export default function Support() {
  const { user } = useAuth()
  const [activeTab, setActiveTab] = useState(user ? 'orders' : 'faqs')
          ...
          {activeTab === 'chat' && (
            <motion.div key="chat" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div className="rounded-3xl text-center py-14 px-8"
                ...
                <button onClick={() => { setSelectedOrderId(null); setIsChatOpen(true); }} id="open-chat-btn"
