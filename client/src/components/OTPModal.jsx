...
import { FiShield, FiX, FiRefreshCw } from 'react-icons/fi';
...
const OTPModal = ({ isOpen, onClose, onSuccess, targetEmail }) => {
  ...
  const handleSubmit = (e) => {
    ...
    setLoading(true);
    // Let the parent handle the verification via their actual API call
    onSuccess(otpString);
    setLoading(false);
  };
        ...
        >
          ...
          <form onSubmit={handleSubmit}>
              ...
              {otp.map((digit, index) => (
                <input
                  ...
                  style={{
                    ...
                    boxShadow: digit ? '0 0 0 2px rgba(245,166,35,0.1)' : 'none'
                  }}
            ...
            <button
              ...
              style={{ width: '100%', justifyContent: 'center', padding: '14px', fontSize: 15, opacity: (loading || otp.join('').length !== 6) ? 0.7 : 1 }}
            >
              {loading ? 'Verifying...' : 'Verify & Apply Changes'}
            </button>
          </form>
          ...
          <div style={{ textAlign: 'center', marginTop: 20 }}>
            <button
              ...
              disabled={sending}
              style={{
                background: 'none', border: 'none', color: 'var(--brand-secondary)', fontSize: 13, fontWeight: 700,
