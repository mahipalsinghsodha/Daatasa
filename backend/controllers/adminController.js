...
exports.getActivityLogs = async (req, res) => {
  try {
    const logs = await AuditLog.find()
      .populate('adminId', 'name email')
      .sort({ createdAt: -1 })
      .limit(500); // Pagination could be added later
    res.json(logs);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
exports.createAdmin = async (req, res) => {
  try {
    ...
    await logAction(req, 'CREATE_ADMIN', 'USER', newAdmin._id, {
      name: newAdmin.name,
      email: newAdmin.email,
      assignedPermissions: newAdmin.permissions
    });
...
exports.updateAdminPermissions = async (req, res) => {
  try {
    ...
    // Security: Super Admin cannot demote themselves accidentally via this route if we are careful
    // But for now, let's just allow the update
    const oldPermissions = targetAdmin.permissions;
    const oldRole = targetAdmin.role;
    ...
    await logAction(req, 'UPDATE_ADMIN_ACCESS', 'USER', targetAdmin._id, {
      previousRole: oldRole,
      newRole: targetAdmin.role,
      previousPermissions: oldPermissions,
      newPermissions: targetAdmin.permissions
    });
...
exports.deleteAdmin = async (req, res) => {
  try {
    ...
    await logAction(req, 'DELETE_ADMIN', 'USER', id, {
      name: target.name,
      email: target.email
    });
